...
```php
Odhiambo\LaravelEntrust\LaravelEntrustServiceProvider::class,
```

3) In the same `config/app.php` and add the following to the `aliases ` array: 

```php
'LaravelEntrust'   => Odhiambo\LaravelEntrust\Facades\LaravelEntrustFacade::class,
```
...
If you don't, then try `php artisan vendor:publish --tag=LaravelEntrust` and, if it does not appear, manually copy the `/vendor/odhiamboatieno/laravel-entrust/src/config/entrust.php` file in your config directory.
